...
#pragma config(Sensor, dgtl8,  quadDriveBL,    sensorQuadEncoder)
...
#pragma config(Motor,  port9,           driveBL,       tmotorVex393_MC29, openLoop, driveRight, encoderPort, dgtl8)
...
void moveLiftWithLogic(int speed, bool overrideHold, bool dampenSpeed, bool overrideBounds){
	...
	if(speed>0) {
		...
		if(currHeight > LIFT_MAX_HEIGHT && !overrideBounds){
			speed = 0;
		}
	} else { //if moving down
		...
		if(currHeight < liftStartingAngle && !overrideBounds) {
			speed = 0;
		}*/
	}
...
void lockLift(){
	moveLiftWithLogic(30,true,true,false);
	return;
	if(!holdActive){
		holdActive = true;
...
void waitForLiftOld(int speed){
	...
	if(speed>=0){
		...
		while(SensorValue[potArm] < LIFT_MAX_HEIGHT){
			moveLiftWithLogic(speed,true,(speed>=ceil(LIFT_MIN_SPEED/LIFT_SLOW_MOD)),false); // the speed must be reset for the auto slow down to work
			wait1Msec(25);
		}
...
void initLift(){
	...
	holdActive = false;
	desiredHeight = SensorValue[potArm];
	liftStartingAngle = SensorValue[potArm];
}
	...
	*/
	...
	while (true)
	{
		...
		*/
		...
		if(vexRT[Btn6U] || vexRT[Btn6UXmtr2] || vexRT[Btn6D] || vexRT[Btn6DXmtr2] || vexRT[Btn8D] || vexRT[Btn8DXmtr2])
			clawControlActive = true;

		if(!vexRt[Btn7U] && !vexRT[Btn7UXmtr2]){
			//Individual Claw Control
			int clawIndivSpeed = 75;
			if(vexRT[Btn7D] || vexRT[Btn5DXmtr2]) {
				clawIndivSpeed *= -1;
			}
			if(vexRT[Btn7L] || vexRT[Btn7LXmtr2]) {
				clawControlActive = false;
				motor[clawL] = clawIndivSpeed;
				motor[clawR] = 0;
			} else if (vexRT[Btn7R] || vexRT[Btn7RXmtr2]) {
				clawControlActive = false;
				motor[clawR] = clawIndivSpeed;
				motor[clawL] = 0;
			} else if(!clawControlActive) {
		...
		if(vexRT[Btn8L] || vexRT[Btn8LXmtr2]){
			resetMotorEncoder(clawL);
			resetMotorEncoder(clawR);
			clawLastPosLeft = 0;
			clawLastPosRight = 0;
		}
