...
#define LIFT_ACC_MOV_RANGE                                                     \
  30                         // amount lift will move before it is held in place
...
#define LIFT_SLOW_MOD                                                          \
  0.8 // how much (0-1] that the lift should be slowed down at bounds
...
// (WARNING: If you wish to reinstate a hold afterwards you must call
// lockLift().
...
void waitForLiftOld(int speed) {
  ...
  if (speed >= 0) {
    ...
    while (analogRead(ANALOG_POT_LIFT) < LIFT_MAX_HEIGHT) {
      moveLiftWithLogic(
          speed, (speed >= ceil(LIFT_MIN_SPEED / LIFT_SLOW_MOD))); // the speed
                                                                   // must be
                                                                   // reset for
                                                                   // the auto
                                                                   // slow down
...
void lockLift() { moveLiftWithLogic(30, true); }

// Sets up all necessary tasks and instance data for the lift
void initLift() { startingAngle = analogRead(ANALOG_POT_LIFT); }
...
void setHoldUp(bool holdUp) { holdUpLift = holdUp; }
