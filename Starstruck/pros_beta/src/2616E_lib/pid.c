...
// getCurrentValFunction: function determining how to get the current value (EX. Encoder reading)
// setCurrentValFunction: function determining what to do with pid output (EX. power motors)
// PIDIteratorCallbackFunction: function that runs when the loop is done executing (EX. turn off motors)
...
// double: range of accepted values to terminate loop, should be relatively low, depending on what you are measuring
int setPIDLoop(unsigned int index, getCurrentValFunction valueGetter, setCurrentValFunction valueSetter, PIDIteratorCallbackFunction PIDIteratorCallback, double kP, double kI, double kD, double integralLimit, double threshold){
  if(index<PID_LOOP_COUNT){ //still can create loops (under max)
    ...
    print("[ELib] ERROR: Tried to create another PID loop, but already too many created\n");
    print("[ELib] Try increasing the PID_LOOP_COUNT in pid.c if you did this on purpose\n");
    return -1;
  }
}
...
// double: max integral limit before reset
// double: range of accepted values to terminate loop, should be relatively low, depending on what you are measuring
int addPIDLoop(getCurrentValFunction valueGetter, setCurrentValFunction valueSetter, PIDIteratorCallbackFunction PIDIteratorCallback, double kP, double kI, double kD, double integralLimit, double threshold){
  int index = setPIDLoop(initializedLoops, valueGetter, valueSetter, PIDIteratorCallback, kP, kI, kD, integralLimit, threshold);
  if(index != -1){
    initializedLoops++;
  }
  return index;
