...
// AUTOSTACKING VARS
const int OVERSHOOT = 0; // How much to overshoot the stack. Pretty much however high we need to release cone
const int PARTY_HAT_THRESH = 50; // How soon before target we start party hat
const int FOURBAR_INTAKE = 0; // Where we pick up cones
const int FOURBAR_EXTAKE = 700; // Where we drop cones (party hat)
const int DROP_THRESH = 0; // How far from FOURBAR_EXTAKE to start dropping cone
const int LOWER_THRESH = 0; // How far from FOURBAR_INTAKE to start lowering lift
...
// NOTE: TO TUNE...
// 1 Get the y = mx+b so that the lift perfectly goes to the height to place cone
// 2 Then figure out how much PARTY_HAT_THRESH can be given to stack ASAP (also using DROP_THRESH)
