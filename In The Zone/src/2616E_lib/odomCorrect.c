...
lines[0] = {0.0, 1524.0, 609.6, 1524.0};
lines[1] = {1524.0, 0.0, 1524.0, 609.6};
lines[2] = {1828.8, 0.0, 3657.6, 1828.8};
lines[3] = {0.0, 1828.8, 1828.8, 3657.6};
lines[4] = {2133.6, 3048, 2133.6, 3657.6};
lines[5] = {3048, 2133.6, 3657.6, 2133.6};
lines[6] = {1219.2, 1828.8, 1828.8, 1219.2};
lines[7] = {1828.8, 1219.2, 2438.4, 1828.8};
lines[8] = {2438.4, 1828.8, 1828.8, 2438.4};
lines[9] = {1828.8, 2438.4, 1219.2, 1828.8};

// setup the line sensors
// TODO: Fill with actual values later
lineSensor lineSensors[4];
lineSensors[0] = {-10, -10, ANALOG_LINE_LEFT_FRONT};
lineSensors[1] = {10, -10, ANALOG_LINE_RIGHT_FRONT};
lineSensors[2] = {-10, 10, ANALOG_LINE_LEFT_BACK};
lineSensors[3] = {10, 10, ANALOG_LINE_RIGHT_BACK};

// returns the sensor number that is on the line
int sensorOnLine(int sensor) {
  return analogGet(lineSensors[sensor].portNum) > WHITE_THRESH;
}
...
double getSlope(line fieldLine) {
...
point getIntersect(line l1, line l2) {
  ...
  if(round(slope1*100)/100 == round(slope2*100)/100)
    return {-1,-1};
  double x = ((((slope1 * l1.x1) - l1.y1) - ((slope2 * l2.x1) + l2.y1))/(slope1-slope2));
  return {x, evalEq(slope1, x, l1.x1, l1.y1)};
}
...
point getPerpProjection(line fieldLine, double x, double y) {
  ...
  if((round(slope*100)/100 == 0.0) && (fieldLine.y2==fieldLine.y1)) {
    return {x, evalEq(slope, x, fieldLine.x1, fieldLine.y1)};
  } else {
    // non vertical lines
    point intersect = getIntersect(fieldLine, {x, y, (x+1), evalEq(slope, (x+1), x, y)})
...
int getClosestLine(int sensorNum) {
  ...
  for(int i = 0; i < 10; i ++) {
    ...
    //could possibly add in a check here to see if the lowest distance is under a certain value
    return lowestIndex;
  }
}
    ...
    for(int i = 0; i < 4; i ++) {
      ...
      if(sensorOnLine(i)) {
        ...
        if(lineNum != -1 && lineNum > 0 && lineNum < 10) {
          point newPos = getPerpProjection(lines[lineNum], getOdomPosX() +
            lineSensors[i].offX, getOdomPosY() + lineSensors[i].offY);
